...
python sample_views_multi.py --transfer=${checkpoint}  --output_path ${p} --prefix ${pp}2 --cfg ${cfg}  --churn ${churn} --n 4 --offset 13 --n_poses 40 --progress
...
for cfg in 2.0; do
    for churn in 1.0; do 
	python sample_views_multi.py --transfer=${checkpoint}  --progress --output_path ${p} --prefix 100_uc_${cfg}_${churn} --unconditional --cfg ${cfg} --churn ${churn} --n 1 --n_poses 40 --steps 100
